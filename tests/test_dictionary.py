...
class Test_TestDictionary(unittest.TestCase):
    ...
    def test_filter_by_anagram(self):
        self.assertEqual(self.dictionary.filter_by_anagram(
            "bar"), ["arb", "bar", "rab"])

    def test_filter_by_length(self):
        self.assertEqual(self.dictionary.filter_by_length(
            start=3), ["arb", "bar", "foo", "rab", "ree"])
        self.assertEqual(self.dictionary.filter_by_length(start=3, end=4), [
                         "arb", "bar", "baza", "foo", "rab", "ree"])

    def test_filter_from_string(self):
        self.assertEqual(self.dictionary.filter_from_string(
            "arb"), ["arb", "bar", "rab"])
        self.assertEqual(self.dictionary.filter_from_string("oof"), ["foo"])

    def test_group_by_length(self):
        self.assertEqual(self.dictionary.group_by_length(), {
                         3: ["arb", "bar", "foo", "rab", "ree"], 4: ["baza"]})

    def test_minimal_string(self):
        self.assertEqual(self.dictionary.minimal_string(), "aabeefoorz")

    def test_random(self):
        for word in self.dictionary.random():
